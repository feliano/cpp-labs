...
/*
  ...
  if(piece != nullptr){
    if(piece->latin1Representation() == 'p' && to_y == BOARD_MAX_INDEX){
      str += ' ';
      str += '(';
      if(promoteTo == NO_PROMOTION){
        str += 'q';
      }else{
        str += promoteTo;
      }
      str += ')';
    }else if(piece->latin1Representation() == 'P' && to_y == 0){
      str += ' ';
      str += '(';
      if(promoteTo == NO_PROMOTION){
        str += 'Q';
...
bool ChessBoard::isStalemate(){
  // if only bishops left, check that at least one of each are on the same colored square
  int whiteBishops = 0;
  int blackBishops = 0;
  int otherPieces = 0;

  int wbOnWhiteSquare = 0;
  int bbOnWhiteSquare = 0;
  int wbOnBlackSquare = 0;
  int bbOnBlackSquare = 0;

  // loop through all pieces and count
  for(int i = 0;i<BOARD_ROW_SIZE;++i){
    for(int j = 0;j<BOARD_ROW_SIZE;++j){
      ChessPiece * piece = state(i,j);
      
      if(piece!= nullptr){

        if(piece->latin1Representation() == 'b'){
        ...
        }else if(piece->latin1Representation() == 'B'){
          ++blackBishops;
  ...
  // only bishops left
  if(wbOnWhiteSquare == 0 && bbOnWhiteSquare > 0 && bbOnBlackSquare == 0){
    return true;
  }

  if(wbOnWhiteSquare > 0 && bbOnWhiteSquare == 0 && wbOnBlackSquare == 0){
    return true;
  }

  if(wbOnBlackSquare == 0 && bbOnBlackSquare > 0 && bbOnWhiteSquare == 0){
    return true;
  }

  if(wbOnBlackSquare > 0 && bbOnBlackSquare == 0 && wbOnWhiteSquare == 0){
    return true;
  }
...
ChessMove ChessBoard::improved_ai_move(bool isBlack){
  ...
  if(blackCapturingMoves.size() != 0){
    ...
    while(it != blackCapturingMoves.end()){
      int moves = potentialMoves(*it,isBlack,true);

      if(moves >= opponentCapturingMoves && moves > 0){
          move_piece(*it);
          return *it;
      }
      ++it;
    }
    // otherwise select a random capturing move
    //std::srand(RANDOM_SEED);
    std::default_random_engine gen(RANDOM_SEED);
    std::shuffle(blackCapturingMoves.begin(), blackCapturingMoves.end(),gen);  
  ...
  if(blackNonCapturingMoves.size() != 0){
    ...
    while(it != blackNonCapturingMoves.end()){
      ...
      int moves = potentialMoves(*it,isBlack,false);
      
      if(moves > opponentCapturingMoves){
          move_piece(*it);
          return *it;
      }
      // UNTIL HERE
    ...
    //std::srand(RANDOM_SEED);
    std::default_random_engine gen(RANDOM_SEED);
    std::shuffle(blackNonCapturingMoves.begin(), blackNonCapturingMoves.end(),gen);  
...
void ChessBoard::game_loop(bool isAIvsAI){
  ...
  while(true){
    ...
    if((capturingMoves(true).size() == 0 && nonCapturingMoves(true).size() == 0) || stalemate == true){
    ...
    if((capturingMoves(false).size() == 0 && nonCapturingMoves(false).size() == 0) || stalemate == true){
      if(isAIvsAI){
        if(stalemate){
          myfile << "\nStalemate " << std::endl;          
        }else{
...
int ChessBoard::potentialMoves(ChessMove cm, bool isWhite, bool isCapturing){
  
  std::vector<ChessMove> vec;
  // change color temporarily to count potential captures
  if(isCapturing){
    chessColorMatrix(cm.to_x,cm.to_y) = !isWhite;

    ChessPiece * piece = state(cm.from_x,cm.from_y);
    state(cm.from_x,cm.from_y) = nullptr;
    /*
    ChessPiece * piece = cm.piece;
    ChessMove temp_move{cm.to_x,cm.to_y,-1,-1,piece,NO_PROMOTION};
    move_piece(temp_move);
    */
    ...
    /*
    temp_move = {-1,-1,cm.from_x,cm.from_x,piece,NO_PROMOTION};
    move_piece(temp_move);
    */
    state(cm.from_x,cm.from_y) = piece;
    chessColorMatrix(cm.to_x,cm.to_y) = isWhite;

  }else{
    ChessPiece * pawn = new Pawn(!isWhite); // could be any temp piece
    ChessMove temp_move{-1,-1,cm.to_x,cm.to_y,pawn,NO_PROMOTION};
    move_piece(temp_move);
    vec = capturingMoves(isWhite);
    ...
    temp_move = {cm.to_x,cm.to_y,-1,-1,pawn,NO_PROMOTION};
    move_piece(temp_move);
    delete pawn;
  }
